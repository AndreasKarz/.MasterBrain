...
Confix manages configuration as code — encrypts `appsettings.json` at rest, resolves secrets from Azure Key Vault, shared config from a git repo, and local per-environment variables.

**File structure per deployable project** (Host/Worker):

```
Host/
├── .confix.project              ← {"environments":["A","UAT","PAV"]}
├── appsettings.json             ← $secret:, $shared:, $local: variable refs
├── variables.A.json             ← Local vars for A environment
├── variables.UAT.json
├── variables.PAV.json
└── confix/components/           ← .confix.component per config section
```

**Root config**: `.confixrc` defines component inputs (dotnet, graphql), providers (NuGet packages, Shared-Config git repo), and variable sources (`shared` from git, `secret` from Key Vault, `local` from `variables.{ENV}.json`).

**Variable providers**:

| Prefix | Source | Example |
|---|---|---|
| `$secret:` | Azure Key Vault (`spc-{env}-confix`) | `"$secret:Fusion.Db.ConnectionString"` |
| `$shared:` | Shared-Config git repo | `"$shared:Common.Security.Authority"` |
| `$local:` | Local `variables.{ENV}.json` | `"$local:Db.DatabaseName"` |

String interpolation: `"{{$shared:Common.OnPremProxy.Url}}api/"`

**CLI workflow**:

| Context | Command |
|---|---|
| Local build | `confix build` (via `confixbuilder.ps1`) |
| CI restore | `confix restore` |
| CI validate | `confix project validate` (per environment) |
| CI build | `confix project build` → encrypted appsettings per env |
| Pipeline decrypt | `dotnet confix decrypt` via `$(ConfixDecryptServiceConnectionName)` |
