...
Create `Startup.cs` and the Confix configuration files:

**Confix files** (read an existing Host like `src/Advisor/src/Host/` for reference):

```
Host/
├── .confix.project              ← {"environments":["A","UAT","PAV"]}
├── appsettings.json             ← With $secret:, $shared:, $local: variable refs
├── appsettings.dev.json         ← Local dev overrides (no Confix variables)
├── variables.A.json             ← Local vars for A environment
├── variables.UAT.json           ← Local vars for UAT environment
├── variables.PAV.json           ← Local vars for PAV environment
└── confix/components/           ← One .confix.component per config section
```
